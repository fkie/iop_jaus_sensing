...
void VisualSensor_ReceiveFSM::sendReportVisualSensorCapabilitiesExtAction(QueryVisualSensorCapabilitiesExt msg, Receive::Body::ReceiveRec transportData)
{
    lock_type lock(p_mutex);
    JausAddress sender = transportData.getAddress();
    RCLCPP_DEBUG(logger, "sendReportVisualSensorCapabilitiesExtAction to %s", sender.str().c_str());
    ReportVisualSensorCapabilitiesExt response;
    std::map<jUnsignedShortInteger, std::shared_ptr<iop::VisualSensor>>::iterator its;
    for (its = p_sensors.begin(); its != p_sensors.end(); its++) {
        response.getBody()->getVisualSensorCapabilitiesList()->addElement(its->second->get_capability_ext());
    }
    sendJausMessage(response, sender);
}
