...
void StillImage_ReceiveFSM::ImageEndpoint::ros_compressed_image_handler(const sensor_msgs::msg::CompressedImage::SharedPtr msg)
{
    ...
    report = ReportStillImageDataExt();
    ReportStillImageDataExt::Body::StillImageDataList::StillImageDataRec rep_rec;
    rep_rec.setSensorID(id);
    ReportStillImageDataExt::Body::StillImageDataList::StillImageDataRec::ImageFrame img_frame;
    ...
    parent->pEvents_ReceiveFSM->get_event_handler().get_queries(QueryStillImageDataExt::ID, queries);
    std::map<jUnsignedByte, urn_jaus_jss_core_Events::CreateEvent::Body::CreateEventRec::QueryMessage>::iterator it;
    for (it = queries.begin(); it != queries.end(); ++it) {
        QueryStillImageDataExt qr;
        ...
        for (unsigned int q = 0; q < qr_len; q++) {
            QueryStillImageDataExt::Body::QueryStillImageDataList::QueryStillImageDataRec* qr_rec;
...
void StillImage_ReceiveFSM::sendReportStillImageDataExtAction(QueryStillImageDataExt msg, Receive::Body::ReceiveRec transportData)
{
    lock_type lock(p_mutex);
    JausAddress sender(transportData.getSrcSubsystemID(), transportData.getSrcNodeID(), transportData.getSrcComponentID());
    std::map<int, std::shared_ptr<ImageEndpoint>>::iterator it;
    for (it = p_sensors.begin(); it != p_sensors.end(); ++it) {
        if (is_requested_ext(it->first, msg) && it->second->is_report_valid) {
            RCLCPP_DEBUG(logger, "sendReportStillImageDataExt for sensor %d to %s", it->first, sender.str().c_str());
            sendJausMessage(it->second->report, sender);
        }
    }
}

void StillImage_ReceiveFSM::sendReportStillImageDataExtInNativeSystemAction(QueryStillImageDataExt msg, Receive::Body::ReceiveRec transportData)
{
    lock_type lock(p_mutex);
    JausAddress sender(transportData.getSrcSubsystemID(), transportData.getSrcNodeID(), transportData.getSrcComponentID());
    std::map<int, std::shared_ptr<ImageEndpoint>>::iterator it;
    for (it = p_sensors.begin(); it != p_sensors.end(); ++it) {
        if (is_requested_ext(it->first, msg) && it->second->is_report_valid) {
            RCLCPP_DEBUG(logger, "sendReportStillImageDataExtInNative for sensor %d to %s", it->first, sender.str().c_str());
            sendJausMessage(it->second->report, sender);
        }
    }
}
...
bool StillImage_ReceiveFSM::isCoordinateTransformSupported(QueryStillImageDataExt msg)
{
    /// Insert User Code HERE
    return false;
}
...
bool StillImage_ReceiveFSM::is_requested_ext(unsigned short sensor_id, QueryStillImageDataExt& msg)
{
    for (unsigned int i = 0; i < msg.getBody()->getQueryStillImageDataList()->getNumberOfElements(); i++) {
        unsigned short req_id = msg.getBody()->getQueryStillImageDataList()->getElement(i)->getSensorID();
        if (req_id == sensor_id) {
