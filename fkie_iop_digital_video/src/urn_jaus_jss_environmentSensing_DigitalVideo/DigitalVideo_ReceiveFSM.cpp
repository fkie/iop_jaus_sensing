...
DigitalVideo_ReceiveFSM::VideoEndpoint::VideoEndpoint(std::shared_ptr<iop::Component> cmp, int id, int type, std::string url, DigitalVideo_ReceiveFSM& parent)
{
    ...
    this->cmp = cmp;
    this->parent = &parent;
    if (type == digital_resource_endpoint::SERVER_TYPE_RTSP && url.find(":") == -1) {
        // QoS for "latched" behavior
        rclcpp::QoS qos(rclcpp::KeepLast(1)); // Keep last message
        qos.transient_local(); // This makes it "latched"
        iop::Config cfg(cmp, "VideoEndpoint");
        rtsp_sub = cfg.create_subscription<std_msgs::msg::String>(url, qos, std::bind(&DigitalVideo_ReceiveFSM::VideoEndpoint::ros_video_rtsp_handler, this, std::placeholders::_1));
    } else {
...
void DigitalVideo_ReceiveFSM::sendReportDigitalVideoSensorCapabilitiesExtAction(QueryDigitalVideoSensorCapabilitiesExt msg, Receive::Body::ReceiveRec transportData)
{
    /// Insert User Code HERE
}
...
void DigitalVideo_ReceiveFSM::sendReportDigitalVideoSensorStatusAction(QueryDigitalVideoSensorStatus msg, Receive::Body::ReceiveRec transportData)
{
    /// Insert User Code HERE
}
...
void DigitalVideo_ReceiveFSM::updateDigitalVideoSensorRecordingStateAction(ControlDigitalVideoSensorRecording msg)
{
    /// Insert User Code HERE
}
...
bool DigitalVideo_ReceiveFSM::isSupported(ControlDigitalVideoSensorRecording msg)
{
    /// Insert User Code HERE
    return false;
}
...
void DigitalVideo_ReceiveFSM::discovered(const std::string& service_uri, JausAddress& iop_address)
{
    ...
    for (std::map<int, std::shared_ptr<VideoEndpoint>>::iterator it = p_endpoints.begin(); it != p_endpoints.end(); ++it) {
        if (it->second->endpoint.server_url.length() > 0) {
